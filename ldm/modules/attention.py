...
class BasicTransformerBlock(nn.Module):
    ...
    def get_attention_map(self, x, context, hint, generated_image):
        x = self.attn1(self.norm1(x), context=context if self.disable_self_attn else None,hint=hint) + x
...
class SpatialTransformer(nn.Module):
    ...
    def get_attention_score(self, x, context, hint, hook_lst, lst_idx, t, generated_image=True, per_time_step=False):
        # note: if no context is given, cross-attention defaults to self-attention
        if not isinstance(context, list):
            context = [context]
        ...
        for i, block in enumerate(self.transformer_blocks):
            ...
            if generated_image:
                # [8, seq_len_q, 1]
                attention_score = block.get_attention_map(x, context=context[i],hint=hint, generated_image=generated_image)
                ...
                if per_time_step:
                    if type(hook_lst[lst_idx][time_group]) == int:
                        hook_lst[lst_idx][time_group] = attention_score
                    else:
                        hook_lst[lst_idx][time_group] += attention_score
