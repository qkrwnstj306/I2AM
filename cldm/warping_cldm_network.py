...
class CustomSpatialTransformer(nn.Module):
    ...
    def get_attention_score(self, x, context, hook_lst, lst_idx, t, generated_image=True, per_time_step=False):
        ...
        for i, block in enumerate(self.transformer_blocks):
            ...
            # [8, HW, hw]
            attention_score = block.get_attention_map(x, context=context[i], generated_image=generated_image)
            if generated_image:
                attention_score = rearrange(attention_score, 'b (h w) s -> b s h w', h=h, w=w)
            else:
                attention_score = rearrange(attention_score, 'b q (h w) -> b q h w', h=h, w=w)
